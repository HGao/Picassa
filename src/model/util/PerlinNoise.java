...
public class PerlinNoise {
    ...
    public static RGBColor greyNoise(RGBColor left, RGBColor right) {
        return new RGBColor(PerlinNoise.noise(left.getRed() + right.getRed(),
                left.getGreen() + right.getGreen(), left.getBlue()
                        + right.getBlue()));
    }
    ...
    public static RGBColor colorNoise(RGBColor left, RGBColor right) {
        return new RGBColor(PerlinNoise.noise(left.getRed() + 0.3, right
                .getRed() + 0.3, 0), PerlinNoise.noise(left.getGreen() - 0.8,
                right.getGreen() - 0.8, 0), PerlinNoise.noise(
                left.getBlue() + 0.1, right.getBlue() + 0.1, 0));
    }

    // JAVA REFERENCE IMPLEMENTATION OF IMPROVED NOISE - COPYRIGHT 2002 KEN
    // PERLIN.
    // This code implements algorithm described in corresponding SIGGRAPH 2002
    // paper.
